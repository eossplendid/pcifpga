...
static int GooseStrToPar(void)
{
	...
	for(nChannel = 0; nChannel < nCHANNEL; nChannel++) 
	{
		for(nIed = 0; nIed < IedNum[nChannel]; nIed++) 
		{
			...
			if(pDev->ChGSGoId[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChGSGoId[%d,%d]:%d, cfg:%s\n", nChannel, nIed, DevPar.ChGSGoId[nChannel][nIed], gooseCfg.szGoID);
			}
			...
			if(pDev->ChGSSmac[nChannel][nIed] != NULL)
			{
				...
				str2Char(pDev->ChGSSmac[nChannel][nIed], &DevPar.ChGSSmac[nChannel][nIed][0]);
				XML_DEBUG("memcpy to DevPar.ChGSSmac[%d,%d]:%s\n", nChannel, nIed, DevPar.ChGSSmac[nChannel][nIed]);
			}
			...
			if(pDev->ChGSSmac2[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChGSSmac2[%d,%d]:%s\n", nChannel, nIed, DevPar.ChGSSmac2[nChannel][nIed]);
				str2Char(pDev->ChGSSmac2[nChannel][nIed], &DevPar.ChGSSmac2[nChannel][nIed][0]);
			}
			...
			if(pDev->ChGSVlId[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChGSVlId[%d,%d]:%d, cfg:%d\n", nChannel, nIed, DevPar.ChGSVlId[nChannel][nIed], gooseCfg.nVlanID);
			}
			...
			if(pDev->ChGSPrio[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChGSPrio[%d,%d]:%d,cfg:%d\n", nChannel, nIed, DevPar.ChGSPrio[nChannel][nIed], gooseCfg.nPriority);
			}
			...
			if(pDev->ChGSDref[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChGSDref[%d,%d]:%s, len:%d\n", nChannel, nIed, DevPar.ChGSDref[nChannel][nIed], strlen(&DevPar.ChGSDref[nChannel][nIed][0]));
			}
...

			{
				...
				for (k = 0; k < 6; k++)
				{
					XML_DEBUG("%02x ", gooseCfg.szDstMac[k]);
				}
				
				XML_DEBUG("\nsrcMac1: ");
				for (k = 0; k < 6; k++)
				{
					XML_DEBUG("%02x ", DevPar.ChGSSmac[nChannel][nIed][k]);
				}

				XML_DEBUG("\nsrcMac2: ");
				for (k = 0; k < 6; k++)
				{
					XML_DEBUG("%02x ", DevPar.ChGSSmac2[nChannel][nIed][k]);
				}
			}
			XML_DEBUG("\nGocbRef:%s\nGoID:%s\nDataSetRef:%s\n",gooseCfg.szGocbRef, gooseCfg.szGoID, gooseCfg.szDataSetRef);
			#endif

			/*在这里添加链表操作*/
			if (strlen(DevPar.ChGSSmac[nChannel][nIed]) > 0)
			{
				plh_parseStandardListInsert(DevPar.ChGSSmac[nChannel][nIed], 1, NULL, &gooseCfg, nChannel);
			}

			if (strlen(DevPar.ChGSSmac2[nChannel][nIed]) > 0)
			{
				plh_parseStandardListInsert(DevPar.ChGSSmac2[nChannel][nIed], 1, NULL, &gooseCfg, nChannel);
			}
		}
	}

	XML_DEBUG("\n\nprint result:\n");
	plh_parseStandardListDisplay();
...
int xml_deviceStrToPar(void)
{
	...
	memset(&smvCfg, 0, sizeof(smvCfg));
	memset(&DevPar, 0, sizeof(DevPar));

	plh_parseStandardListCreate();										/*创建链表*/
	/*1、装置的列表部分，名称、Ip地址、编号，3变量*/
	if(pDev->DevName != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevName:%s\n",DevPar.DevName);
	}
	...
	if(pDev->DevIp != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevIp:%s\n",DevPar.DevIp);
	}
	...
	if(pDev->DevNo != NULL)
	{
		...
		XML_DEBUG("memcpy to DevPar.DevNo:%d\n",DevPar.DevNo);
	}
	...
	if(pDev->DevVer != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevVer:%s\n",DevPar.DevVer);
	}	
	...
	if(pDev->DevLoIp != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevLoIp:%s\n",DevPar.DevLoIp);
	}	
	...
	if(pDev->DevSnMask != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevSnMask:%s\n",DevPar.DevSnMask);
	}
	...
	if(pDev->DevGway != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevGway:%s\n",DevPar.DevGway);
	}
	...
	if(pDev->DevReIp != NULL) {
		...
		XML_DEBUG("memcpy to DevPar.DevReIp:%s\n",DevPar.DevReIp);
	}

	if(pDev->DevRePort != NULL) 
	{
		DevPar.DevRePort = strtoul(pDev->DevRePort, NULL, 0);
		XML_DEBUG("memcpy to DevPar.DevRePort:%d\n",DevPar.DevRePort);
	}
	...
	if(pDev->DevSubN != NULL)
	{
		...
		XML_DEBUG("memcpy to DevPar.DevSubN:%d\n",DevPar.DevSubN);
	}
...
//		DevPar.ChTrTime[nChannel] = 0;
		if(pDev->ChTrTime[nChannel] != NULL)
		{
			...
			XML_DEBUG("memcpy to DevPar.ChTrTime[%d]:%d\n",nChannel, DevPar.ChTrTime[nChannel]);
		}
		#endif
		...
		if(pDev->ChPacket[nChannel] != NULL)
		{
			...
			XML_DEBUG("memcpy to DevPar.ChPacket[%d]:%d\n",nChannel, DevPar.ChPacket[nChannel]);
		}
		...
		if(pDev->ChAcsLocation[nChannel] != NULL)
		{
			...
			XML_DEBUG("memcpy to DevPar.ChAcsLocation[%d]:%s\n",nChannel, DevPar.ChAcsLocation[nChannel]);
		}
	...
	for(nChannel = 0; nChannel < nCHANNEL; nChannel++) {
		...
		for(nVlan = 0; nVlan < nVLAN; nVlan++) {
			if(pDev->ChVlanNo[nChannel][nVlan] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChVlanNo[%d][%d]:%d\n",nChannel, nVlan, DevPar.ChVlanNo[nChannel][nVlan]);
			}
	...
	for(nChannel = 0; nChannel < nCHANNEL; nChannel++) {
		for(nIed = 0; nIed < nIED; nIed++) {
			if(pDev->ChIedName[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedName[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedName[nChannel][nIed]);
			}
			...
			if(pDev->ChIedMac[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedMac[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedMac[nChannel][nIed]);
			}
			...
			if(pDev->ChIedType[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedType[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedType[nChannel][nIed]);
			}
			...
			if(pDev->ChIedBay[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedBay[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedBay[nChannel][nIed]);
			}
			...
			if(pDev->ChIedMan[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedMan[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedMan[nChannel][nIed]);
			}
			...
			if(pDev->ChIedDesc[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedDesc[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedDesc[nChannel][nIed]);
			}
			...
			if(pDev->ChIedBip1[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedBip1[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedBip1[nChannel][nIed]);
			}
			...
			if(pDev->ChIedBip2[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChIedBip2[%d,%d]:%s\n", nChannel, nIed, DevPar.ChIedBip2[nChannel][nIed]);
			}
		}
	}

	printf("IedNum[0] = %d, IedNum[1] = %d\n", IedNum[0], IedNum[1]);
	...
	for(nChannel = 0; nChannel < nCHANNEL; nChannel++) 
	{
		for(nIed = 0; nIed < IedNum[nChannel]; nIed++) 
		{
			...
			if(pDev->ChMuType[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChMuType[%d,%d]:%d\n", nChannel, nIed, DevPar.ChMuType[nChannel][nIed]);
			}
			...
			if(pDev->ChMuSmac[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChMuDmac[%d,%d]:%s\n", nChannel, nIed, DevPar.ChMuSmac[nChannel][nIed]);

				str2Char(pDev->ChMuSmac[nChannel][nIed], &DevPar.ChMuSmac[nChannel][nIed][0]);
				#if 0
				printf("Notice:ChMuSmac[%d,%d]:%x-%x-%x-%x-%x-%x\n", nChannel, nIed,
											DevPar.ChMuSmac[nChannel][nIed][0], DevPar.ChMuSmac[nChannel][nIed][1],
											DevPar.ChMuSmac[nChannel][nIed][2], DevPar.ChMuSmac[nChannel][nIed][3],
											DevPar.ChMuSmac[nChannel][nIed][4], DevPar.ChMuSmac[nChannel][nIed][5]);
				#endif
			}
			...
			if(pDev->ChMuVlId[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChMuVlId[%d,%d]:%d, smvCfg:%d\n", nChannel, nIed, DevPar.ChMuVlId[nChannel][nIed], smvCfg.nVlanID);
			}
			...
			if(pDev->ChMuPrio[nChannel][nIed] != NULL)
			{
				...
				XML_DEBUG("memcpy to DevPar.ChMuPrio[%d,%d]:%d, svCfg:%d\n", nChannel, nIed, DevPar.ChMuPrio[nChannel][nIed],smvCfg.nPriority);
			}
			...
			if(pDev->ChMuDesc[nChannel][nIed] != NULL) {
				...
				XML_DEBUG("memcpy to DevPar.ChMuDesc[%d,%d]:%s\n", nChannel, nIed, DevPar.ChMuDesc[nChannel][nIed]);
			}

			/*在这里添加链表操作*/
			plh_parseStandardListInsert(DevPar.ChMuSmac[nChannel][nIed], 0, &smvCfg, NULL, nChannel);
		}
	}
	GooseStrToPar();
...
static int HandleNodeGOOSE(xmlDocPtr xmlDOC, xmlNodePtr nodeGoose, int nChannel, int nIed)
{
	...
	nodeCur = nodeGoose;
	XML_DEBUG("HandleNodeGOOSE,nodeCur-name:%s\n",nodeCur->name);
	/*先解析GOOSE属性*/
	...
	while(attrPtr != NULL) {						//attrPtr属性指针不空
		...
		if(!xmlStrcmp(attrPtr->name, BAD_CAST "datasetRef")) { 		//attrPtr属性名称是"mac"
			...
			xmlFree(vAttr);
			XML_DEBUG("TY-3012S xmlDoc ChGSADref[%x]:%s\n", nChannel, pDev->ChGSADref[nChannel][nIed]);
		}		
		if(!xmlStrcmp(attrPtr->name, BAD_CAST "goID")) { 		//attrPtr属性名称是"mac"
			...
			xmlFree(vAttr);
			XML_DEBUG("TY-3012S xmlDoc ChGSAGoId[%x]:%s\n", nChannel, pDev->ChGSAGoId[nChannel][nIed]);
		}
...
static int HandleNodeGOOSEList(xmlDocPtr xmlDOC, xmlNodePtr nodeGoose, int nChannel, int nIed)
{
	...
	while(nodeCur != NULL) {					//IED节点不空
		if(!xmlStrcmp(nodeCur->name, (const xmlChar*)"GOOSE")) {	//节点名称是IED
			XML_DEBUG("find a node GOOSE to handle\n");
...
int xml_parseConfigXml(char* fileName)
{
	...
	while(NULL != testCur)
	{
		XML_DEBUG("parse xmlFile node name is :%s\n", testCur->name);
...
int xml_saveRecordFile(int chNo)
{
	...
	{
		XML_DEBUG("xml_saveRecordFile chNo error, chNo:%d, lineNo:%d\n", chNo, __LINE__);
		return  -1;
	}
...
//	info = &curFile;
	...
	if (diskno < 0)
	{
		XML_DEBUG("xml_delDiskRecord:diskno < 0 ---lineNo:%d\n", __LINE__);
		return -1;
	}
	Cur = xmlDiskPtr[chNo].diskx[diskno];								//nodeDev指向"File"
	printf("xml_delDiskRecord, chNo:%d, position:%d\n", chNo, diskno);
	
	//XML_DEBUG("xml_delDiskRecord cur name is :%s, child name is :%s\n", Cur->name, Cur->xmlChildrenNode->name);

	if (Cur == NULL)
	{
		XML_DEBUG("xml_delDiskRecord:Cur = NULL, disk record not found\n");
		return -1;
	}
	...
	{				
		...
		{
			XML_DEBUG("find a File node to del, cur name :%s, ---lineNo:%d\n", Cur->name, __LINE__);
			xmlNodePtr tempNode;
			tempNode = Cur->next;
	...
	XML_DEBUG("find a File node to del, end, ---lineNo:%d\n", __LINE__);
...
int xml_recreateDiskRecord(int chNo, curFileInfo *info)
{
	...
	XML_DEBUG("xml_recreateDiskRecord, chNo:%d, diskno:%d, diskName:%s, ---lineNo:%d\n", chNo, diskno, diskName, __LINE__);
...
//	XML_DEBUG("xml_refreshNodeContent packCnt:%ld,cnt:%s, endTm:%s\n",curFile.packCnt, cnt, curFile.attrEndTm);
...
static int HandleNodeFile(xmlDocPtr xmlDOC, xmlNodePtr nodeFile, int disk, int chNo)
{
	...
	while(attrPtr != NULL) {						//attrPtr属性指针不空
		if(!xmlStrcmp(attrPtr->name, BAD_CAST "no")) {		//attrPtr属性名称是"no"
			...
			{
				//XML_DEBUG("not the last file node ,continue ---lineNo:%d\n", __LINE__);
				return -1;								//不是最后一个节点
			}

			info->attrNo = atol((char *)vAttr);
			//XML_DEBUG("TY-3012S xmlDocinfo->attrNo:%d, lineNo:%d\n", info->attrNo, __LINE__);
		}
		...
		if(!xmlStrcmp(attrPtr->name, BAD_CAST "startTime")) { 		
			...
			//XML_DEBUG("TY-3012S xmlDoc info->attrStartTm:%s,\n", info->attrStartTm);
		}
	...
	while(nodeCur != NULL) {						//nodeCur节点不空，LocIp节点不空
		...
		{	
			...
			xmlFree(strFile);
			XML_DEBUG("TY-3012S xmlDoc pRec->record[%d]->chNo[%d]:%s\n", disk, count, pRec->record[disk]->chNo[count]);
		} 
		else if(!xmlStrcmp(nodeCur->name, (const xmlChar*)"size")) 
		{
			...
			xmlFree(strFile);
			XML_DEBUG("TY-3012S xmlDoc pRec->record[%d]->size[%d]:%s\n", disk, count, pRec->record[disk]->size[count]);
		} 
		#endif
...
static int HandleNodeDiskAmount(xmlDocPtr xmlDOC, xmlNodePtr nodeAmount, int disk, int chNo)
{
	...
	xmlFree(strCom);
	XML_DEBUG("TY-3012S xmlDoc pRec->amount:%s! disk:%d  lineNo:%d\n", pRec[chNo]->diskAmount[disk], disk, __LINE__);
...
static int HandleNodeDisk(xmlDocPtr xmlDOC, xmlNodePtr nodeDisk, int disk, int chNo)
{
	...
	{
		XML_DEBUG("disk value is error! handleNodeDisk failed!\n");
		return -1;
	}
	...
	{		//nodeCur节点指针不空
		...
		{
			XML_DEBUG("find disk Amount node, lineNo:%d\n", __LINE__);
		...
		if (flag == 0)
		{
			XML_DEBUG("currentDisk is :%c, node disk is :%d, no need to parse  ---lineNo:%d\n", temp, disk, __LINE__);
			break;
		}
...
static int HandleNodeDiskList(xmlDocPtr xmlDOC, xmlNodePtr nodeDiscList, int chNo)
{
	...
	XML_DEBUG("disklist cur name is :%s, child name is :%s\n", Cur->name, Cur->xmlChildrenNode->name);
...
int xml_parseRecordFile(int chNo, char *filename)
{
	...
	if(xmlDOC == NULL) {				//对xml文件进行解析不成功
		XML_DEBUG("TY-3012S xmlDoc parse no successfully!\n");
		retv = -1;				//对xml文件进行解析不成功
		return retv;
	}						//对xml文件进行解析成功
	XML_DEBUG("TY-3012S xmlDoc parse successfully!\n");
	...
	if(Cur == NULL) {				//是一个空xml文件
		XML_DEBUG("TY-3012S xmlDoc empty document!\n");
	...
	if(xmlStrcmp(Cur->name, BAD_CAST "RECORD")) {	//判断节点名字，寻找"RECORD"，使用xmlStrcmp函数
		XML_DEBUG("TY-3012S xmlDoc root node no RECORD!\n");
	...
	while(Cur != NULL) {				//文件的第一个子结点不空，执行
		...
		if(!xmlStrcmp(Cur->name, (const xmlChar*)"CurrentDisk")) {	//寻找"CurrentDisk"，相等!
			//处理Amount节点数据，处理所有数据
			XML_DEBUG("find node CurrentDisk!\n");
...
int xml_checkRecordFile(int chNo)
{
	...
	if (info->attrNo == 0)
	{
		XML_DEBUG("info->attrNo : %d, ---lineNo:%d\n", info->attrNo, __LINE__);
		return 0;
	}
	...
	if (info->status == 0)
	{
		XML_DEBUG("xml_checkRecordFile, file not finish saving\n");
	...
	{
		XML_DEBUG("xml_checkRecordFile, file finished \n");
		return 0;
	}
}
#endif
...
void	xml_fileRecordInit()
{
	...
	if ((recFd = open(XML_RECORD_FILE1A, O_RDONLY)) == -1)
	{
		...
		{
			XML_DEBUG("xml_createNewRecord, lineNo:%d\n", __LINE__);
		...
		{
			XML_DEBUG("xml_parseRecordFile, lineNo:%d\n", __LINE__);
			close(recFd);
	...
	else
	{
		...
		if (xml_parseRecordFile(0, XML_RECORD_FILE1A) < 0)
		{
			...
			if ((recFd = open(XML_RECORD_FILE1C, O_RDONLY)) == -1)
			{
				...
				XML_DEBUG("fileRecord.xml lose, check disk to recovery, it will take a long time, please wait...");
				#if 0
				sleep(1);
	...
	if ((recFd = open(XML_RECORD_FILE2A, O_RDONLY)) == -1)
	{
		...
		{
			XML_DEBUG("xml_createNewRecord chNo:2, lineNo:%d\n", __LINE__);
			/*A,B下都没有,判断为新机器,创建新记录*/
