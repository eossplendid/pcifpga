...
MsgLedStatus		*AppLedStatus[nCHANNEL];
...
static int parseFifoFrameData(int chNo)
{
	...
	static unsigned int gooseSvPacketCnt = 0;
	int ret, no;
	int jumpLen;
	
	static MsgProcessLayer msg;
	MsgSvInfo svMsg;
	Flags *appFlag;
	...
	while(1)
	{
		qLength = appFlag->bufferStatus.rear - appFlag->bufferStatus.analyz;
		#if 0
		APP_DEBUG("qLenth:%ld\ndatas:", qLength);
		APP_DEBUG("%x %x %x %x %x %x\n", *appFlag->bufferStatus.analyz,
		...
		pHead = (unsigned int *)appFlag->bufferStatus.analyz;
		if (Ntohs(*pHead) == FRAME_HEADE)		/*找到了帧头1a2b3c4d*/
		{
			pLength = (unsigned short*)(pHead + 1);
			//APP_DEBUG("pLength:%x, ---lineNo:%d\n", *pLength, __LINE__);
			...
			#if 0
			pPayload = pDataTime + 16;													
			pFrameState = (unsigned short *)(pDataTime + (Ntohs(*pLength) - 2));				/*读取报文类型和异常标志*/
			#else
			jumpLen = Ntohs(*pLength) - 18;											/*framestate2字节,time16字节,剩下payload*/
			pPayload = (unsigned char *)((long long *)pDataTime + 2);
			/*为减少指针移动的次数,这里进行类型转换,先按8字节跳转,
				再对8取余,将尾巴几个字节补上*/
			pFrameState = (unsigned short *)(((unsigned char *)((long long *)pPayload + (jumpLen / 8))) + (jumpLen % 8));
			#endif
			//pFCS = (unsigned int *)(pFrameState + 1);											/*暂不做FCS校验*/
			//pTail = pFCS + 1;																/*帧尾,如果不对，假设数据丢失，从datatime开始找帧头*/

			#if 0
			if (*pTail != FRAME_TAIL)
			{
				appFlag->bufferStatus.analyz += 4;		
				/*帧头对但是帧尾不对,跳过帧头4字节重新解码,还不对就在外层if里面逐字节判断*/
				printf("wrong frame tail pointer +4\n");
				continue;
			}
			#endif
			...
			#if 0
			/*检验各指针位置是否正确*/
			APP_DEBUG("pHead:%08x, pLength:%08x, pTime:%08x  \n", *pHead, *pLength, *((unsigned int *)pDataTime));
			APP_DEBUG("pPayload:%x %x %x %x\n",*pPayload, *(pPayload+1), *(pPayload+2), *(pPayload+3));
			APP_DEBUG("pType:%x\n", *pType);
			APP_DEBUG("pFrameState:%08x, pFCS:%08x, pTail:%08x\n",*pFrameState, *pFCS, *pTail);			
			#endif
			...
			if (((style & 0x10) == 0) && ((style & 0x20) == 0))			/*既不是smv,也不是goose，分析下一帧,1<<4,1<<5*/
			{
				...
				appFlag->bufferStatus.analyz = (unsigned char *)(pFrameState + 5);		/*下一帧,state后10字节*/
				continue;
			}
			...
			if (appFlag->stopParse == 0)
			{
				...
				if (appFlag->queueFull == 1)
				{
					if (plh_getMsgQueueStatus(no) > 0)
					{
						...
						gooseSvPacketCnt = 0;
						/*给分析指针赋值*/
						msg.packets[gooseSvPacketCnt] = appFlag->bufferStatus.analyz;
						gooseSvPacketCnt++;
						/*sv和goose 报文个数统计*/
						#if 1
						if ((style & 0x10) != 0)
						{
							appFlag->svPackets++;
							msg.type[gooseSvPacketCnt] = MSG_PROCESS_SVTYPE;
						#if 0
						...
						#else
			//				if (memcmp(svCondition, SvCondition.dstMac, 14) == 0)			/*这样比较效率高*/
						#endif
							{
								if (SvCondition.svEnable > 0)
								{
									...
									{
										svUploadInfoSet(&appFlag->bufferStatus.analyz, (Ntohs(*pLength) + 14), 1);
									}
								}
							}
						}
						else if ((style & 0x20)  != 0)
						{
							...
							msg.type[gooseSvPacketCnt] = MSG_PROCESS_GOOSETYPE;
						}
						#endif
	...
	//						memcpy(svCondition+12, pPayload+14+4,2);
							...
							if ((strcmp(dstMac, appFlag->svCondition.dstMac) == 0) && 
									...
									(appFlag->svCondition.appID == appId))
						#else
		//					if (memcmp(svCondition, appFlag->svCondition.srcMAC, 14) == 0)			/*这样比较效率高*/
						#endif
							{
								if (SvCondition.svEnable > 0)
								{
									svUploadInfoSet(&appFlag->bufferStatus.analyz, (Ntohs(*pLength) + 14), 0);
								}
							}
						#endif
					}
					...
					if (gooseSvPacketCnt == MAX_PROCESS_NUM)
					{
						/*10个包给分析线程发一次消息*/
						msg.mtype = TYPE_PROCESSLAYER;
			...
			#endif
			appFlag->bufferStatus.analyz = (unsigned char *)(pFrameState + 5); 				/*下一帧*/
...
unsigned char * app_getBufferStartPtr(int no, int chNo)
{
	if (no == 0)						/*bufferA*/
	{
		return AppFlag[chNo].bufferStatus.buf;
	}
	else if (no == 1)					/*bufferB*/
	{
		return AppFlag[chNo].bufferStatus.bufBStart;
	}
	else if (no == 2)					/*bufferC*/
	{
		return AppFlag[chNo].bufferStatus.bufCStart;
	}
...
{
	MsgAppBuf msg;
	unsigned long qLength = 0, len =0;			//记录大小,count每10MB存一次,total每300M换一个文件,qLenth > 25MB就停分析
	...
	static unsigned int idleTimes = 0;
	...
	while(1)
	{
		...
		if (len == 0)
		{
			...
			if ((idleTimes > 500) && (idleTimes <= 1500))
			{
			...
			else if (idleTimes > 1500)				/*等了15秒还是没数据过来,结束当前文件存储,加文件尾*/
			{
				if (appFlag->newFile != 0)
				{
			...
			#endif
			usleep(400);						/*没读到数据就延迟10ms,释放cpu资源*/
			continue;
		}
		else if (len >0)
		{
		...
		if((appFlag->bufferStatus.rear - appFlag->bufferStatus.buf) > (MBYTES_30))
		{
			...
			//	printf("bufA overflow , warning ---lineNo:%d\n", __LINE__);
			}
			else
			{
		...
		else if((appFlag->bufferStatus.rear - appFlag->bufferStatus.buf) > (MBYTES_20))
		{
			...
			//	printf("bufC overflow , warning ---lineNo:%d\n", __LINE__);
			}
			else
			{
				/*如果2区数据还没存,先存了2区数据*/
		...
		{
			appFlag->stopParse = 1;
			APP_DEBUG("chNo:%d,buf busy , stop parse ---lineNo:%d\n", chNo, __LINE__);		
			#if 0
...
int app_Init(pthread_t *id1, pthread_t *id2)
{
	...
	#if 1
	chNo[0] = 1;
	AppFlag[0].bufferStatus.buf = (unsigned char *)malloc(sizeof(unsigned char) * MAXQSIZE);
	...
	#if 1
	chNo[1] = 2;
	AppFlag[1].bufferStatus.buf = (unsigned char *)malloc(sizeof(unsigned char) * MAXQSIZE);
