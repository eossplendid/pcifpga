...
#else
/*以int为单位*/
#define MBYTES		0x40000				/*1MB / 4*/
#define MBYTES_30	0x780000			/*30MB / 4*/
#define MBYTES_20	0x500000			/*20MB / 4*/
#define MBYTES_10	0x280000			/*10MB / 4*/
#define MAXQSIZE 	0x800000			/*32MB / 4*/
#endif
...
typedef struct
{
	unsigned int *buf;				/*缓冲区指针*/
	unsigned int *rear;				/*尾指针,指向缓冲区当前数据尾部*/
	unsigned int *analyz;				/*分析指针，指向整包包头*/
	unsigned int *bufBStart;			/*缓冲2区的起始位置*/
	unsigned int *bufCStart;			/*缓冲3区的起始位置*/
