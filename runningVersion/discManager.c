...
void dm_msgDiscMngSend(int quest, char *param1, int param2, long param3, int param4)
{
	...
	msgsnd(DmQid, &msg, (sizeof(MsgQueueBuf)-sizeof(long)), 0);
...
static void dm_add_fileTail(unsigned long packets, int chNo)
{
	...
	//tail.packetCount = packets 
	tail.packetCount = (packets<<24)|((packets<<8)&0x00FF0000)|((packets>>8)&0x0000FF00)|(packets>>24);
...
{
	static unsigned char timeStr[16];
	TimeStr tStr;
...
//	FileHeader head;
	...
	sprintf(info->filename, "XXX-%d-%d-%s", devNo, chNo+1, info->attrStartTm);
...
//	int count = 0;
	...
	while(1)
	{
		...
		if (msgrcv(DmQid, &msg, (sizeof(MsgQueueBuf)-sizeof(long)), type, 0) == -1)
		{
		...
		switch (msg.msgRequest)
		{
			case MSGQ_RQST_SAVE:
			{
				...
				ch = msg.msgParam2-1;
				#if 1
				if ((len = write(WriteDiskFd[ch], (const unsigned char *)app_getBufferStartPtr(msg.msgParam4, ch), msg.msgParam3*4)) < 0)
				{
