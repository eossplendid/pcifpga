			...
			if (appFlag->stopParse == 0)
			{
				...
				if (appFlag->queueFull == 1)
				{
					if (plh_getMsgQueueStatus(no) > 0)
					{
						...
						/*给分析指针赋值*/
						msg.packets[gooseSvPacketCnt[no]] = appFlag->bufferStatus.analyz;
						gooseSvPacketCnt[no]++;
						/*sv和goose 报文个数统计*/
						#if 1
						if ((style & 0x10) != 0)
...
#endif
{
	...
	/*测试代码,虚拟文件模拟读包*/
	sprintf(testName, "/root/hp/gsPct%d", chNo);
	printf("app_read start, chNo:%d,testName:%s\n", chNo, testName);
	testFd = open(testName, O_RDONLY);
	/*****************************************/
	...
	while(1)
	{
		...
		sleep(1);
		len = read(testFd, appFlag->bufferStatus.rear, MBYTES_5M);			/*将完整的消息内容送入Driver*/
		#if 0
...
int app_Init(pthread_t *id1, pthread_t *id2)
{
	...
	memset(&SvCondition, 0, sizeof(SvUploadCondition));
