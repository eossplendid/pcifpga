...
//消息变量
#define	MSG_BUF_LEN	0x40		//定义消息的最大长度,字节
#define	CH_BUF_LEN	0x10000		//定义接收报文帧的最大长度,双字
#define	READ_BUF_LEN	0x10000		//定义每次读取报文帧的长度,双字
...
unsigned int		Ch1Buffer[CH_BUF_LEN];//64K双字
unsigned int		Ch2Buffer[CH_BUF_LEN];//64K双字
...
#define	PCAP_HEAD_LEN	6	//共计06双字
typedef struct pcapHeader	//共计24字节
{
	unsigned int pcapHead;	//0xD4C3B2A1;
	unsigned int pcapMajor;	//0x02000400;
	unsigned int pcapZone;	//0x00000000;
	unsigned int pcapFigs;	//0x00000000;
	unsigned int pcapSnap;	//0xffff0000;
	unsigned int pcapType;	//0x00000100;
}pcapFileHeader;

pcapFileHeader	*AppFileHeader;
unsigned int pcapHeadBuf[PCAP_HEAD_LEN];

static int SetPcapHeadBuf(void)
{
	AppFileHeader = (pcapFileHeader *)pcapHeadBuf;
	memset(pcapHeadBuf, 0x0, 4*PCAP_HEAD_LEN);
	AppFileHeader->pcapHead	 = 0xD4C3B2A1;
	AppFileHeader->pcapMajor = 0x02000400;
	AppFileHeader->pcapZone  = 0x00000000;
	AppFileHeader->pcapFigs  = 0x00000000;
	AppFileHeader->pcapSnap  = 0xffff0000;
	AppFileHeader->pcapType  = 0x01000000;
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	//while(1) {
		...
		AppReadBuffer->Byte = READ_BUF_LEN;			//256K字节,每次最大应该检查64K双字
		memcpy(Ch1Buffer, MsgBuffer, MSG_BUF_LEN);
		len1 = read(fd, Ch1Buffer, READ_BUF_LEN);		//每次从驱动读取64K双字
		#if 0
		if(len1 >= 0) {
			printf("TY-3012S App read ch1: %d!", len1);
			for(i = 0; i < len1; i++) {
				if((i % 0x04) == 0) printf("\n%04x:", i);
				printf(" %08x", Ch1Buffer[i]);
			}
			printf("\n");
		...
		#if 1
		if(len1 >= 0) {
			printf("TY-3012S App write ch1: %d!\n", len1);
			WriteDiskFd = open("PacpAppFile", O_RDWR|O_CREAT);
			if (WriteDiskFd == -1) {
				printf("TY-3012S App open PacpAppFile error!\n");
				return -1;//严重问题,打开磁盘文件错误
			}
			printf("TY-3012S App open PacpAppFile successfully!\n");
			if(len1 >= 4) {
				i = write(WriteDiskFd, pcapHeadBuf, 4*PCAP_HEAD_LEN);
				printf("TY-3012S App write ch1 %d ok!\n", i);
			}
			if(len1 >= 4) {
				i = write(WriteDiskFd, (Ch1Buffer + 2), 4*(len1 - 4));
				printf("TY-3012S App write ch1 %d ok!!\n", i);
			}
			close(WriteDiskFd);
		} else {
		...
		#if 1
		if(len2 >= 0) {
			printf("TY-3012S App write ch2: %d!\n", len2);
			WriteDiskFd = open("PacpAppFile", O_RDWR|O_CREAT);
			if (WriteDiskFd == -1) {
				printf("TY-3012S App open PacpAppFile error!\n");
				return -1;//严重问题,打开磁盘文件错误
			...
			if(len2 >= 4) {
				i = write(WriteDiskFd, pcapHeadBuf, 4*PCAP_HEAD_LEN);
				printf("TY-3012S App write ch2 %d ok!!\n", i);
			}
			if(len2 >= 4) {
				i = write(WriteDiskFd, (Ch2Buffer + 2), 4*(len2 - 4));
				printf("TY-3012S App write ch2 %d ok!!\n", i);
			}
			close(WriteDiskFd);
		} else {
