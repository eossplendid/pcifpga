			...
			case MSG_RQST_SVUPLOAD:
			{
				...
				{					
					if (msg.packet[i] != NULL)
					{
						...
						{
							printf("SvUpload write socket failed, errno:%s ---lineNo:%d\n", strerror(errno), __LINE__);
							app_setSvEnable(0);
							close(sockfd);
...
/*goose告警上报消息发送*/
void bs_gooseWarningMsgsend()
{
	static unsigned long session = 1;
	session++;
	...
	warning.chNo =1;
	warning.appId = 22;
	warning.dataTime1 = 0x20121205;
	warning.dataTime2 = 0x113828;
	memset(warning.dstMac, 0x12, 6);
	memset(warning.srcMac, 0xa0, 6);
	warning.almCatg = 0x02;
	warning.msgAckTag = 0;
	warning.MsgAlarmCode = 0xffffffff;
	warning.tail1 = Htons(0x0909); 
	warning.tail2 = Htons(0xD7D7); 
	//sleep(1);
	printf("snd  gswarnings\n");
	...
	while(1) 
	{
		...
		switch(MsgId) 
		{
			...
			case MS_SEND_SMVMS_REQ:
			{
				...
				BS_DEBUG("TY-3012S MS_SEND_SMVMS_REQ MsgTail:     %08x!\n", MsgTail);
				if(MsgTail != 0x0909D7D7) {
					close(ClientSockfd);	//消息的定位帧尾4字节出现错误，关闭连接，并重新建立连接!
					BS_DEBUG("TY-3012S MS_SEND_SMVMS_REQ decodes tail error1: %d!\n", ClientSockfd);
					break;
				}

				/*此处应先查看是否有录播文件正在上传,如果有，拒绝sv上传请求*/
				/*这部分可能有问题,后面再调*/
				SmvCmd->Reserved = 1;					/*对应装置上的Enable位*/
			...
			case MS_STOP_SMVMS_REQ:
			{
				...
				MsgTail = Ntohs(cmd->tail1 * 0x10000) + Ntohs(cmd->tail2);
				BS_DEBUG("TY-3012S MS_STOP_SMVMS_REQ ServerRecListCmd->MsgTail:     %08x!\n", MsgTail);
				if(MsgTail != 0x0909D7D7) {
					close(ClientSockfd);	//消息的定位帧尾4字节出现错误，关闭连接，并重新建立连接!
					BS_DEBUG("TY-3012S recordFileTcp decodes tail error1: %d!\n", ClientSockfd);
					break;
...
static void *netServerInit(void *arg)
{
	...
	if(listen(TcpSockfd, 3) < 0) {	//侦听队列的长度=3!
